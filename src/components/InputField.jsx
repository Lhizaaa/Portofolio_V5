...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      w-full p-4 rounded-xl bg-surface-2 text-fg placeholder-transparent
      focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2
      focus:ring-offset-base transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgb(var(--c-accent)/0.4)] border-accent"
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-muted transition-colors group-hover:text-accent">
        <Icon className="w-5 h-5" />
        <label
          htmlFor={field}
          className={`
            absolute left-12 top-1/2 transform -translate-y-1/2 text-muted text-sm transition-all duration-300
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-accent peer-focus:text-sm
          `}
        >
          {label}
        </label>
